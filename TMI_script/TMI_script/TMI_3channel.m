...
gamma_on = 1;
for n  = [5,7,11,21,23,25]
    for sigmaz = [3]
        for RL1_on = [1]
            for gamma_1 =[1]
                % RL1_on = 0;
                filename_front = 'H2A-rsEFFP2-E&mito-rsFastLime&KDEL-Drp_0';
                num = num2str(n);
                filename_last = '.vsi - C488.tif';
                mkdir('result');
                imgname = [filename_front,num,filename_last];
                finter = 2;
                confname = '20250408_3channel_H2A_mito_MAP4';%荧光关闭曲线文件名
                rsFPs = 3;%荧光团个数
                frame = 30; %frame number used for SACD reconstruction
                FWHM = 3.5;
                FWHM_TMI = 3.5;
                iter1 = 5; %iteration times of pre decovnolution
                iterTMI = 10;
                % FWHM4gauss1 = g;%用于模糊TMImask的高斯半高宽，因为拆解出的图像比较断裂，需要做高斯模糊来弥补一下
                % FWHM4gauss2 = g;
                % FWHM4gauss3 = g;%两个拆解通道都做高斯
                ifsub = 0;%是否减背景
                backg = 4;%减背景系数，越大减的越少
                baseline = 0;%图像基线信号，直接在图像数据上减除
                sigma = [1,1,sigmaz];%对raw高斯去噪
                sigma2 = 0;%对衰减信号高斯去噪
                par1 = 1;
                TMIon = 1;%是否进行通道拆解，如果设为0就只输出SACD结果
                sparse_on = 0;%是否使用sparseSACD，一般不用启用
                diff_param = 0;
                % RL1_on = 0;
                %% SACD RL1
                ...
                stack4TMI = zeros([size(imgstack,1),size(imgstack,2),frame]);
                if sigma(3) ~=0
                ...
                for i = 1:frame
                    ...
                    stack4TMI(:,:,i) = deconvlucy(imgstack_gauss(:,:,i),IpsfTMI,iterTMI); % the first RL deconvolution
                ...
                if TMIon == 1
                    ...
                    parfor ii = 1 : imageWidth
                        for jj = 1: imageHeight
                            ...
                            if stack1(ii,jj,1) < 500/65535 %
                                ...
                                [predicted_vals1, ~, exitflag1, ~] = fmincon(@(coeffs)signalseparation3fmincon(ch1(1:frame), ch2(1:frame),ch3(1:frame), curve4s, coeffs), ... % obj. fun.
                                    [0.33,0.33,0.33], ... % initial guess
                                    [], [], ... % inequalities (N/A)
                    ...
                    imwritestack(double(abs(stack4TMI.*65535)),[imgfolder,'/',num2str(n),'_RLon',num2str(RL1_on),'gauss_RL.tif']);

                    % for g  = [2]
                    FWHM4gauss1 = 2;%用于模糊TMImask的高斯半高宽，因为拆解出的图像比较断裂，需要做高斯模糊来弥补一下
                    FWHM4gauss2 = 2;
                    FWHM4gauss3 = 2;%两个拆解通道都做高斯
                    ...
                    if FWHM4gauss3~=0
                        composition_image(:,:,3) = imgaussfilt(composition_image(:,:,3), FWHM4gauss3);
                    end
