folderPath = 'm';
tifFiles = dir(fullfile(folderPath, '*C488.tif'));
fileNames_green = {tifFiles.name};
fileNames_green = string(fileNames_green');
...
for n = 1:37
    ...
    imTruth = redRAW(:,:,1);
    mSACD = imreadstack(['result/20250724m/',num2str(n),'_frame20/','RLon1_sigma110_fwhm2.52.5_iter3_3_finter4gaus11_order1_1_TMISACD_ch2.tif']);
    imSplit = mSACD(:,:,1);
...
for n = 1:37
    ...
    redRAW = imreadstack([imgname]);
    imTruth = redRAW(:,:,1);
    TMI = imreadstack(['result/20250724m/',num2str(n),'_frame20/','TMIch2.tif']);
    imSplit2 = TMI(:,:,1);
