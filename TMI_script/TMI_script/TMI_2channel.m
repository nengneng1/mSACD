...
%% initiation
clc;clear;close all;
folder = '20250512_3colors';
gamma_on = 1;
for n  = [1:2:53]
    filename_front = 'ER-mstaygold&lenti-tomm20-sky62A_30%_';
    num = num2str(n);
    ...
    data = data(:,:,1:20);
    skip = 20;
    frame = size(data,3)/skip; % Number of SR frames
    for f=1:frame
        f
        imgstack = data(:,:,(f-1)*skip+1:f*skip);
        MIP_ = max(imgstack,[],3);
        MIP(:,:,f) = double(MIP_./max(MIP_(:)));
        for gamma_1 =[1]
            RL1_on = 1;
            sparseTMIon = 0;
            mkdir('result');
            finter = 2;
            confname = '30ms_ER_tomm';%荧光关闭曲线文件名
            rsFPs = 2;%荧光团个数
            ...
            ifsub = 1;%是否减背景
            backg = 2;%减背景系数，越大减的越少
            baseline = 0;%图像基线信号，直接在图像数据上减除
            %%
            sigma = [0.5,0.5,3];%对raw高斯去噪
            sigma2 = 0;%对衰减信号高斯去噪
            %%
            par1 = 1;
